// using zoro's thing for testing
...
class FlxGameJolt
{
	 ...
	 */
	static inline var URL_API:String = "https://api.gamejolt.com/api/game/v1_2/";
	 ...
	 static function parseDataBatch(e:Event):Void
	{
		...
		for (string in stringArray)
		{
			...
			if (string.length > 1)
			{
				...
				if (temp[0] == 'key')
					keys++;
				returnMap.set(temp[0]+suffix, temp[1]);
			}
		}
